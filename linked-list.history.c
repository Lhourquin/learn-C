...
void insert_in_middle(List *list, int new_number, Element *previous_elem);
...
int main(void){
	...
	printf("%d\n", my_list->prime_number->number);
	Element *previous_elem = my_list->prime_number;
	printf("previous_elem: %p\n", previous_elem->next);
	insert_in_middle(my_list, 13, previous_elem->next);
...
void insert_in_middle(List *list, int new_number, Element *previous_elem){
	...
	if(list == NULL || previous_elem == NULL){
		perror("previous_elem: NULL\n");
		exit(EXIT_FAILURE);
	}
	
	if(previous_elem != NULL){
		while(actual != previous_elem){
			actual = actual->next;	
		}
